...
public class Work05 : MonoBehaviour
{
    ...
    private void OnDestroy()
    {
        ...
        buttonPlay.onClick.RemoveListener(PlayClicked);
    }
    ...
    void RecClicked()
    {
        ...
        isRecording = !isRecording;
        buttonRec.GetComponentInChildren<Text>().text = (!isRecording ? "録画開始" : "録画中");
    ...
    void PlayClicked()
    {
        ...
        isPlaying = !isPlaying;
        buttonPlay.GetComponentInChildren<Text>().text = (!isPlaying ? "再生開始" : "再生中");
    ...
    void StopPlay()
    {
        ...
        for (int i = 0; i < Players.Length; i++)
        {
            if (Players[i].GetComponent<Rigidbody>() != null)
    ...
    void StartRecord()
    {
        ...
        {
            var r = Recorders[i];
            r.transform.localPosition = startTrans[i].Pos;
            r.transform.localRotation = startTrans[i].Rot;
